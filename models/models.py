...
class Model(torch.nn.Module):
    ...
    def forward_rgb_features(self, x):
        if self.rgb_backbone_name == 'vit_base_patch14_dinov2.lvd142m':
            self.rgb_backbone.eval()
            x = self.rgb_backbone.forward_features(x)
            feat = x[:, 1:].permute(0, 2, 1).view(x.shape[0], -1, 37, 37)
        else:
            x = self.rgb_backbone.patch_embed(x)  # in torch.Size([B, 3, 224, 224]) out torch.Size([B, 784, 768])
            x = self.rgb_backbone._pos_embed(x)  # out torch.Size([B, 785, 768])
            x = self.rgb_backbone.norm_pre(x)
            # if self.rgb_backbone.grad_checkpointing and not torch.jit.is_scripting():
            #     x = checkpoint_seq(self.blocks, x)
            # else:
            #     x = self.rgb_backbone.blocks(x)  # out torch.Size([B, 785, 768])
            x = self.rgb_backbone.blocks(x)  # out torch.Size([B, 785, 768])
            x = self.rgb_backbone.norm(x)

            # feat = x[:, 1:].permute(0, 2, 1).view(1, -1, 28, 28)  # out torch.Size([1, 768, 28, 28])
            feat = x[:, 1:].permute(0, 2, 1).view(x.shape[0], -1, 28, 28)
        return feat

    def forward(self, rgb=None, xyz=None, out_type='rgb+xyz'):
        if out_type == 'rgb+xyz':
            ...
            # rgb_features = B 768 28 28 xyz_features= B 1152 128
...
def fps(data, number):
    """
        ...
        number int
    """
    # maybe rewrite with https://blog.csdn.net/weixin_42707080/article/details/105279415
...
class Group(torch.nn.Module):
    ...
    def forward(self, xyz):
        ...
        # fps the centers out
        # center is points torch.Size([1, 1024, 3]), center_idx is the index of points torch.Size([1, 1024])
        center, center_idx = fps(xyz.contiguous(), self.num_group)  # B G 3
        # knn to get the neighborhood
        _, idx = self.knn(xyz, center)  # B G M in，B G XYZ, B G IDX OUT
        assert idx.size(1) == self.num_group
        assert idx.size(2) == self.group_size
        ori_idx = idx
        # an idx from 0-bathcsize shape as (batchsize, 1, 1),value*num_points, to give unique label for each point
        idx_base = torch.arange(0, batch_size, device=xyz.device).view(-1, 1, 1) * num_points
        idx = idx + idx_base
        idx = idx.view(-1)
        # pick up those points in groups
        ...
        return neighborhood, center, ori_idx, center_idx  # output: B G M 3 center : B G 3
...
class Block(nn.Module):
    #   B G+1 transToken, B G+1 transToken in
...
class Encoder(torch.nn.Module):
    ...
    def forward(self, point_groups):
        ...
        feature_global = torch.max(feature, dim=2, keepdim=True)[0]  # bs*g, 256, n in, pick the maximum point?
        # max pooling as skip connect and broadcast
...
class PointTransformer(torch.nn.Module):
    def __init__(self, group_size=128, num_group=1024, encoder_dims=384):
        ...
        if self.encoder_dims != self.trans_dim:  # means lack of cls and pos?
            self.cls_token = torch.nn.Parameter(torch.zeros(1, 1, self.trans_dim))
            self.cls_pos = torch.nn.Parameter(torch.randn(1, 1, self.trans_dim))
            self.reduce_dim = torch.nn.Linear(self.encoder_dims, self.trans_dim)
    ...
    def forward(self, pts):
        if self.encoder_dims != self.trans_dim:
            B, C, N = pts.shape  # 1 3 224*224-zero
            pts = pts.transpose(-1, -2)  # B N 3
            # divide the point cloud in the same form. This is important
            # nei= B G M C3 cen = B G C3 oriidx= B G M 1 center_idx B G 1
            ...
            group_input_tokens = self.encoder(neighborhood)  # B G M C3 in B G N (B G encoToken)
            group_input_tokens = self.reduce_dim(group_input_tokens)  # (B G transToken)
            # prepare cls (1, 1, transToken) to (B, 1, transToken)
            cls_tokens = self.cls_token.expand(group_input_tokens.size(0), -1, -1)
            cls_pos = self.cls_pos.expand(group_input_tokens.size(0), -1, -1)
            # add pos embedding from center position B G 3 to B G transToken
            pos = self.pos_embed(center)
            # final input
            x = torch.cat((cls_tokens, group_input_tokens), dim=1)  # (B G+1 transToken)
            pos = torch.cat((cls_pos, pos), dim=1)  # (B G+1 transToken)
            # transformer
            ...
            group_input_tokens = self.encoder(neighborhood)  # B G N (B G Token)
            ...
            # x=B C G, C=1152
            feature_list_num = len(feature_list)
            for i in range(feature_list_num):
                if i == 0:
                    x = feature_list[i]
                else:
                    x = torch.cat((x, feature_list[i]), dim=1)  # 1152
            return x, center, ori_idx, center_idx
